...
export default async (req) => {
  ...
  try {
    ...
    if (body.action === "start") {
      return new Response(JSON.stringify({welcomeUrl: S3_BASE + "RT_intro.mp4"}), {status:200, headers:H});
    }
    ...
    if (body.action === "find") {
      ...
      return new Response(JSON.stringify({clipId: id, clipPersona: CLIPS[id].persona, clipUrl: clipUrl(id), clipSummary: CLIPS[id].sum}), {status:200, headers:H});
    }
